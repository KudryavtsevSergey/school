...
public class ExcelGeneratorService implements IGenerator {
    ...
    public OutputStream generateClassPupilListDocument(OutputStream os, Teacher teacher, Clazz clazz, List<Pupil> pupilList) throws ServiceException {
        ...
        XSSFSheet sheet = workbook.createSheet("Список учеников "+clazz.getNumber()+clazz.getLetterMark());
        int rowNum = 0;
            ...
            for (int i = startIndex; i < endIndex + 1; i++){
                ...
                for (SubjectInSchedule subject : scheduleList){
                    ...
                    createCellWithValue(timeRow,
                            subject.getSubjectBySubjectId().getName() + ", " +
                                    subject.getPlace() + ", " +
                                    String.valueOf(subject.getClazzByClassId().getNumber()) +
                                    subject.getClazzByClassId().getLetterMark(),
                            DayOfWeekEnum.valueOf(subject.getDayOfWeek()).getValue() ,
                            style);
                }
                rowNum++;
            }
        ...
        for (int i = startIndex; i < endIndex + 1; i++){
            ...
            for (SubjectInSchedule subject : scheduleList){
                ...
                createCellWithValue(timeRow,
                                    subject.getSubjectBySubjectId().getName() + ", " +
                                    subject.getPlace() + ", " +
                                    subject.getTeacherByTeacherId().getLastName() + " " +
                                    subject.getTeacherByTeacherId().getFirstName().charAt(0) + ". " +
                                    subject.getTeacherByTeacherId().getPathronymic().charAt(0) + ".",
                                    DayOfWeekEnum.valueOf(subject.getDayOfWeek()).getValue() ,
                                    style);
            }
            rowNum++;
    ...
    public OutputStream generateFullScheduleDocument(OutputStream os, List<SubjectInSchedule> subjectInScheduleList, List<LessonTime> lessonTimeList, List<Teacher> teacherList) throws ServiceException {
        ...
        for(Teacher teacher:teacherList){
            ...
            for(SubjectInSchedule subject: subjectInScheduleList){
                if(Objects.equals(subject.getTeacherByTeacherId().getUserByTeacherId().getUserId(), teacher.getUserByTeacherId().getUserId())){
            ...
            for(SubjectInSchedule subject: teacherSchedule){
                int cellPosition = (DayOfWeekEnum.valueOf(subject.getDayOfWeek()).getValue() - 1) * 10;
                for(int i = 0; i < lessonTimeList.size();i++){
                    if(subject.getBeginTime().getTime() == lessonTimeList.get(i).getStartTime().getTime()){
                        cellPosition += i + 1;
                    }
                }
                createCellWithValue(teacherRow,subject.getSubjectBySubjectId().getName() + ", " +
    ...
    public OutputStream generateMarksDocument(OutputStream os, Subject subject, List<Mark> markList, List<Pupil> pupilList, Clazz clazz, List<Date> lessonDateList) throws ServiceException {
        ...
        for(Pupil pupil:pupilList) {
            ...
            for(Mark mark: markList){
                if(mark.getPupilByPupilId().getUserByPupilId().getUserId()==(pupil.getUserByPupilId().getUserId())){
                    pupilMarks.add(mark);
                }
            }
            ...
            for(Mark pupilMark: pupilMarks){
                ...
                setMarkFont(pupilMarkCellStyle, MarkType.valueOf(pupilMark.getType()));
                switch (MarkType.valueOf(pupilMark.getType()).getMeaning()){
                    case 1: {
